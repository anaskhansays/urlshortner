 ...
 * 
 * @author Anas Khan <anas@anaskhansays.co.in> 
 * @copyright Copyright (c) 2021, Anas Khan
 * @url https://anaskhansays.co.in/
 */ 
...
class URLShortner
{
    ...
    protected function urlExistsInDB($url){
        $query = "SELECT short_code FROM ".self::$tbl_name." WHERE long_url = :long_url LIMIT 1";
    ...
    protected function insertUrlInDB($url, $code){
        $query = "INSERT INTO ".self::$tbl_name." (long_url, short_code, created) VALUES (:long_url, :short_code, :timestamp)";
    ...
    protected function validateShortCode($code){
        $rawChars = str_replace('|', '', self::$characters);
        return preg_match("|[".$rawChars."]+|", $code);
    }

    protected function getUrlFromDB($code){
        $query = "SELECT id, long_url FROM ".self::$tbl_name." WHERE short_code = :short_code LIMIT 1";
        $stmt = $this->pdo->prepare($query);
        $params=array(
            "short_code" => $code
        );
    ...
    protected function incrementCounter($id){
        $query = "UPDATE ".self::$tbl_name." SET hits = hits + 1 WHERE id = :id";
        $stmt = $this->pdo->prepare($query);
        $params = array(
            "id" => $id
        );
